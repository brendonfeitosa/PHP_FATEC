...
if(isset($_GET['id'])){
	$id = $_GET['id'];
	$mysql_query = "SELECT * FROM contatos WHERE id={$id}";//pegar dados do banco
	$result = $conn->query($mysql_query); 
	$row = mysqli_fetch_assoc($result); //pegando o resultado de uma linha apenas com um array associativo
...
if(isset($_POST['enviar'])) {
	...
	$mysql_query = "UPDATE contatos SET nome='{$nome}', email='{$email}', datanasc='{$datanasc}' 
							WHERE id='{$id}'";
