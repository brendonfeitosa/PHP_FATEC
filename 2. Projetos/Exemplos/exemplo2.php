  ...
  if (isset($_GET['enviar'])) { //isset verifica se a variavel existe
    $p1 = $_GET["p1"];
    $p2 = $_GET["p2"];
    ...
    if (!empty($p1) && !empty($p2) && is_numeric($p1) && is_numeric($p2) && $p1 >= 0 && $p2 >= 0) {
      ...
      $flag_msg = false; //erro
      $msg = "Dados incorretos, preencha o formulario corretamente!";
    }
  }
  ...
  <body>
    ...
    <div class="container">
      <form method="GET">
        ...
        <a href="exemplo2.php"><button type="button" class="btn btn-primary mb-2" name="limpar">Limpar</button></a>
      </form>
      <?php
      if (!is_null($flag_msg)) {
        if ($flag_msg) {
